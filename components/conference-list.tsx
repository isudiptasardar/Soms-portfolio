...
export default function ConferenceList({ conferencesByYear, years }: ConferenceListProps) {
  ...
  const filteredData = useMemo(() => {
    ...
    if (!searchQuery && selectedTypes.length === 0 && selectedCountries.length === 0) {
      return { conferencesByYear, years }
    }
    ...
    years.forEach((year) => {
      if (!conferencesByYear[year]) return
      ...
      const conferences = conferencesByYear[year].filter((conference) => {
        ...
        const matchesSearch =
          ...
          (conference.location && conference.location.toLowerCase().includes(searchQuery.toLowerCase())) ||
          (conference.country && conference.country.toLowerCase().includes(searchQuery.toLowerCase()))
        ...
        const matchesCountry =
          selectedCountries.length === 0 || (conference.country && selectedCountries.includes(conference.country))
  ...
  }, [searchParams, conferencesByYear, years, searchQuery, selectedTypes, selectedCountries])
